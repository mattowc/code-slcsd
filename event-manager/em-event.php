	...
	function output($format, $target="html") {	
		...
		foreach($placeholders[1] as $key => $result) {
			...
			switch( $result ){
				...
				/*
				Changes by Jonathon McDonald
				#_EVENTGALS returns number of girls attending
				#_EVENTGUYS returns number of guys attending
				#_EVENTSLOTSGALS returns number of slots available for girls
				#_EVENTSLOTSGUYS returns number of slots available for guys
				*/
				...
				case '#_EVENTGALS':
					...
					foreach( $people as $person ) {
						$user_sex = bp_get_profile_field_data( 'field=Sex&user_id='.$person->ID );
						if( $user_sex == 'Female' ) {
							$i++;
						}
					}
				...
				case '#_EVENTSLOTSGUYS':

					foreach( $this->get_tickets() as $EM_Ticket ) {
						if( $EM_Ticket->ticket_name == 'Guys' )  {
							$replace = $EM_Ticket->get_available_spaces();
						}
					}

					break;

				case '#_EVENTSLOTSGALS': 
					foreach( $this->get_tickets() as $EM_Ticket ) {
						if( $EM_Ticket->ticket_name == 'Girls' )  {
							$replace = $EM_Ticket->get_available_spaces();
						}
					}
				...
				/*
				END Jonathon McDonald's changes
				*/
